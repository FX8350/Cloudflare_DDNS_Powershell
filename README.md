...
例: `.\cloudflare_dns.ps1 -Hostname "hogehoge.net","test.hogehoge.net" -TTl 120 -Token "hogefuga" -y -UseIPv6 -IPv6Index 2 -LogLevel Info`
...
* タスクスケジューラに登録する際は、以下のいずれかで登録しないとウインドウが一瞬表示されて大変うっとおしいです
   1. 「ユーザーがログオンしてるかどうかにかかわらず実行する」を指定する
   2. 権限を`SYSTEM`にする
   3. JavaScriptやVBScriptでヘルパーとなる起動スクリプトを書き、そこから起動する
* 起動時やログイン時をトリガーにすると、まだネットがつながってない状態で処理を行おうとしてエラーになる時がります。<br>
特にスリープ復帰時、問答無用で即実行しようとするみたいなので高確率にエラーが記録されます。<br>
その場合は`-Dealy`オプションを引数に追加して適当な秒数待つようにしてください。
...
デフォルトでは指定なし(無効)になっています。IPv6関係の他のオプションはこのオプションが指定されていないとエラーになります。<br>
このスクリプトではWindowsからIPv6アドレスを取得しています。<br>
一般的なIPv6環境であれば取得できると思いますが、自分の環境でしか試せていません。<br>
不具合が出る場合は`-Ipv6Source`オプションを変更してください。
...
### -Delay <秒>
指定した秒数だけスクリプトの冒頭で処理を停止します。
タスクスケジューラーであまりに早く起動してネットワークエラーが出る場合に適当に指定してください。
コンソールには表示されますが、ログには記載されません。

### -ZoneId
~~編集するZone IDを指定します。  
無くてもAPIトークンから自動で取得しますが、指定するとリクエスト一回分通信を節約できます。~~<br>
どのみちゾーン名を確認するのにリクエスト一回必要なので廃止しました
